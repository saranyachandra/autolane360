                  ...
                  <div class="tab-pane active" id="USA">
                    <table class="table table-responsive">
                      ...
                      <tbody>
                        <?php 
                            $i = 1;
                            foreach ($customer_detail as $details) {
                        ?>
                        <tr>
                          <td><?php echo $i;?></td>
                          <td><?php echo $details->business_name;?> </td>
                          <td><?php echo $details->business_phone." / ".$details->business_email;?> </td>
                                                                      
                          <td><?php echo $details->business_country;?></td>
                          <td><?php echo $details->membership_id;?></td>
                          <td>
                            <input type="hidden" id="carpart_id" value="<?php echo $details->carpart_reg_id;?>">
                            <select name="category" class="carpart_status form-control">
                              <option  value="PENDING" <?php if(isset($details->membership_status) && ($details->membership_status == "PENDING" )){ echo "selected"; } else { echo ""; } ?> >PENDING </option>
                              <option value="APPROVED" <?php if(isset($details->membership_status) && ($details->membership_status == "APPROVED" )){ echo "selected"; } else { echo ""; } ?> >APPROVED </option>
                              <option value="SUSPENDED" <?php if(isset($details->membership_status) && ($details->membership_status == "SUSPENDED" )){ echo "selected"; } else { echo ""; } ?> >SUSPENDED </option>
                            </select>
                          </td>
                          <td>
                            <a class="btn btn-info" href="<?php echo base_url('admin/carpart_customer_view_details/'.$details->agent_cus_id); ?>"> View </a> 
                            <a class="btn btn-danger delete_customer" style="padding: 5px;" href="<?php echo base_url('admin/carpart_customer_delete/'.$details->carpart_reg_id); ?>" >delete</a>
                          </td>
                        </tr>
                        <?php $i++; } ?>
                      </tbody>
